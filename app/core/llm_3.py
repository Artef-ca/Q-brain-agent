...
class LLM:
    ...
    def _sanitize_prompt(self, prompt: str, trace: Optional[Trace] = None) -> str:
        # SAFE LOGGER: Check if trace exists before getting ID
        t_id = trace.trace_id if trace else "NO_TRACE"
    ...
    def text(self, prompt: str, trace: Optional[Trace] = None) -> str:
        ...
        # Sanitize Output (FIXED: Added trace parameter here so it passes down!)
    ...
    def json(self, prompt: str, schema_hint: str = "", trace: Optional[Trace] = None) -> dict:
        ...
        # We NO LONGER sanitize the prompt here. 
