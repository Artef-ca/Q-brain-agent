    ...
    def run(self, trace: Trace, question: str, is_pro_mode: bool = False) -> dict[str, Any]:
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=10) as global_executor:
            ...
            if plan.get("intent") == "greeting":
                ...
                followup_future = global_executor.submit(self._generate_followups, followup_context, greeting_response, trace, context_scope="")
            ...
            if plan.get("intent") == "system_capabilities":
                ...
                logger.info(f"--- [RootAgent] [Trace: {trace.trace_id}] Spawning background threads for Trends, Insights, and Schemas... {schema_future} ---")

                recent_trends = trends_future.result()
                trending_insights = insights_future.result() # or "" if you disabled it
                ...
                ### AVAILABLE DATA DICTIONARY (Tables & Columns):
                {schema_catalog}
                ...
                TASK:
                1. if user question is about available datasets/tables/columns provide the requested information from given schema_catalog
                2. Write a crisp, welcoming introduction explaining your core capabilities.
                3. If the user is curious about what is happening in Qiddiya, provide insights along with the date when they were captured.
                4. If the user wants to know how to start, give them examples of what people are talking about based on User Trends.
                ...
                # 3. Generate dynamic suggestions based on EVERYTHING we just found
                followup_context = f"Available Data:\n{schema_catalog}\n\nUser Trends:\n{recent_trends}\n\nTrending Insights:\n{trending_insights}"
                followup_instruction = "Based ONLY on the available data tables, recent queries, and insights, suggest 3 exact, highly interesting follow-up questions the user can click."
            ...
            if plan.get("needs_bq"):
                ...
                for attempt in range(1, max_meta_retries + 1):
                    ...
                    search_query = contextualized_question if attempt == 1 else question
