...
class TelemetryWriter:
    ...
    def write_trace(self, trace: Trace, status: str, final_intent: str, answer_len: int) -> None:
        try:
            # Fallback to current time if the agent crashed before setting end_datetime
            ...
            row = {
                ...
                "start_datetime": trace.metadata.get("start_datetime", current_time),
                "end_datetime": trace.metadata.get("end_datetime", current_time),
                # -----------------------------
                
                "created_at": current_time,
                "entity_state": json.dumps(trace.metadata.get("entity_state", {})) 
            }
        ...
        try:
            ...
            for llm in trace.llm_calls:
                rows.append({
                    ...
                    # --- NEW TIMESTAMP COLUMNS ---
                    "start_datetime": llm.get("start_datetime"),
                    "end_datetime": llm.get("end_datetime"),
                    # -----------------------------
    ...
    def write_model_armor_audit(self, trace: Trace) -> None:
        if not hasattr(trace, 'model_armor_audit') or not trace.model_armor_audit:
            return

        try:
            rows = []
            for audit in trace.model_armor_audit:
                rows.append({
                    "trace_id": trace.trace_id,
                    "session_id": trace.session_id,
                    "user_id": trace.user_id,
                    "phase": audit.get("phase", "unknown"),
            ...
            errors = self.client.insert_rows_json(self._table("model_armor_audit"), rows)
            if errors:
                logger.error(f"BQ Insert Error (model_armor_audit): {errors}")
        except Exception as e:
            logger.error(f"Failed to prepare model_armor_audit telemetry: {e}")
    ...
    def write_all_logs(self, trace: Trace, status: str = "OK", final_intent: str = "unknown", answer_len: int = 0) -> None:
        try:
            ...
            self.write_model_armor_audit(trace)
